...
export default function CompanyTabs() {
 ...
 useEffect(() => {
  ...
  return (
    <div className='p-6 '>
    <div className=" bg-white min-h-screen rounded p-6  space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between bg-white p-4 rounded-lg">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">ABC Trucking</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="px-4 py-2 text-sm text-white bg-[#111827] hover:bg-gray-800 rounded-lg">
            ✏️ Edit Company
          </button>
        </div>
      </div>
      ...
      {/* Tab Navigation */}
      <div className="flex space-x-4 bg-brand2 py-3 px-3 overflow-x-auto rounded-md">
        {tabs.map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded ${
              activeTab === tab ? 'bg-cyan-400 text-white' : 'text-gray-600 hover:text-gray-800'
            }`}
          >
            {tab}
          </button>
...
const AccessReportSection = () => {
  const accessLogs = [
    { name: 'John Smith', role: 'Admin', lastAccess: '2025-07-14 10:45 AM' },
    { name: 'Sarah Lee', role: 'Manager', lastAccess: '2025-07-13 4:30 PM' },
    { name: 'Ali Raza', role: 'Viewer', lastAccess: '2025-07-13 9:10 AM' },
  ];
  return (
    <div className="p-6 rounded-lg border space-y-4 w-full bg-white shadow-sm">
      <h2 className="text-lg font-semibold text-gray-800">Access Report</h2>
      <div className="space-y-3">
        {accessLogs.map((log, index) => (
          <div key={index} className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">{log.lastAccess}</p>
          </div>
        ))}
      </div>
    </div>
